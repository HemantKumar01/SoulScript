...
def draw_logo(c, x, y, width=150, height=30):
    ...
    try:
        ...
        if (
            LOGO_SVG_CONTENT
            and LOGO_SVG_CONTENT.strip()
            and not LOGO_SVG_CONTENT.strip().startswith("<!--")
        ):
...
def draw_enhanced_wrapped_text(
    c,
    x,
    y,
    text,
...
def create_pdf_from_json(json_data, filename):
    ...
    system_info = (
        f"Generated by SoulScript System | {datetime.now().strftime('%B %d, %Y')}"
    )
    ...
    y = draw_enhanced_wrapped_text(
        c, margin + 20, notice_y - 30, notice_text, max_chars=80
    )
    ...
    for priority, section_key, section_title, items in sorted_sections:
        ...
        has_content = any(
            ...
            and item.get("value", "").lower() not in ["unclear", "not provided"]
            for item in items
        )
        ...
        for label, values in grouped_items.items():
            ...
            for value in values:
                ...
                y = draw_enhanced_wrapped_text(
                    c, margin + 20, y, f"• {value}", font_size=10, color=text_color
                )
    ...
    if "graph" in json_data:
        ...
        y = draw_subsection_header(
            c, margin, y, "Assessment Scores (Ranked by Severity)"
        )
    ...
    c.drawString(
        margin,
        30,
        "End of Report - All information confidential and for therapeutic use only",
    )
    c.drawRightString(
        width - margin, 30, f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
